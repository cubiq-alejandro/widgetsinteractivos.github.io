...
/******************   HELPERS    *******************/
function calculateDistance(a, t, n, r) {
    var s = deg2rad(n - a),
        h = deg2rad(r - t),
        M = Math.sin(s / 2) * Math.sin(s / 2) + Math.cos(deg2rad(a)) * Math.cos(deg2rad(n)) * Math.sin(h / 2) * Math.sin(h / 2),
        d = 2 * Math.atan2(Math.sqrt(M), Math.sqrt(1 - M));
    return parseInt(6371 * d)
}
...
function displayMarkers(newPosition) {
    ...
    for (i in items) {
        if (!isEmpty(newPosition)) {
            items[i].distancia = calculateDistance(newPosition.lat, newPosition.lng, items[i].latitud, items[i].longitud);
        }
    }
